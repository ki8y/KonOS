...
powercfg -h off
:: Taskbar tweaks
cls && echo 🛈 Removing taskbar bloat...
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Windows Feeds" /v "EnableFeeds" /t REG_DWORD 0 /f >nul 2>&1


::
:: Disables animations and whatnot
::
...
:: Disables copilot
echo 🛈 Attempting to disable copilot...
...
echo 🛈 Enabling file extensions and access to hidden files...
...
:: Im getting tired of commenting all of these
...
:: Disabling telemetry (ngl i stole this code from chris titus, here's his links:
:: YouTube: https://www.youtube.com/@ChrisTitusTech
:: Winutil: https://christitus.com/winutil-install/
...
echo 🛈 Disabling Windows Ink features...
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Power\ModernSleep" /v "AdaptiveRefreshRate" /t REG_DWORD /d 0 /f >nul 2>&1
...
:: (This has been disabled because it fucks stuff up on 24h2 pretty badly.)