:: Hi, thanks for using Kon OS. It means a lot to me that I can help your PC run just a little bit better than before.
:: Credits: just me tbh,
:: Inspiration: Chris Titus, Privacy is Freedom, Quaked
...
if %build% GEQ 26100 (
    ...
    title Redstone 5 Detected
    echo                                             ╭───────────────────────────╮
    echo                                             │ Windows 11 [33m1809[0m detected^^! │
    echo                                             ╰───────────────────────────╯
    echo.
...
powershell -Command "Checkpoint-Computer -Description 'Pre-KonOS Restore Point'"
echo Restore point created!
echo If needed, it's named: "Pre-KonOS Restore Point".
timeout /t 3 >nul
goto UCPD
...
REM OOShutUp10
echo.
echo 💾 Installing OOShutUp10 (and the config)...
...
echo.
echo ⬇️ Intalling ALT+TAB scripts...
...
echo.
echo ⬇️ Installing Bluetooth scripts...
...
REM Wifi Control
echo.
echo ⬇️ Installing Wi-fi Control...
...
echo.
echo ⬇️ Installing DDU...
call "%~dp0Modules\Scripts\dduInstaller.bat"
:: NVCleanInstall
echo.
echo ⬇️ Installing NVCleanInstall...
...
mkdir "%SYSTEMDRIVE%\Kon OS\Display Drivers\NVIDIA\1) NVCleanInstall"
set "location=%SYSTEMDRIVE%\Kon OS\Display Drivers\NVIDIA\1) NVCleanInstall"
curl -s -L "%file%" -o "%location%\%name%"
...
echo.
echo ⬇️ Installing NVIDIA Profile Inspector...
call "%~dp0Modules\Scripts\nvProfileInspectorInstaller.bat"
:: AMD
echo.
echo ⬇️ Downloading AMD web shortcut...
set "file=https://raw.githubusercontent.com/ki8y/Tweaks/main/GPU/AMD/AMD.url"
set "name=AMD.url"
mkdir "%SYSTEMDRIVE%\Kon OS\Display Drivers\AMD\1) Drivers"
set "location=%SYSTEMDRIVE%\Kon OS\Display Drivers\AMD\1) Drivers"
curl -s -L "%file%" -o "%location%\%name%"
echo.
...
echo.
echo ⬇️ Installing Timer Resolution...
...
mkdir "%SYSTEMDRIVE%\Kon OS\Modules\Timer Resolution"
set "location=%SYSTEMDRIVE%\Kon OS\Modules\Timer Resolution"
curl -s -L "%file%" -o "%location%\%name%"
...
set "name=bench.ps1"
mkdir "%SYSTEMDRIVE%\Kon OS\Modules\Timer Resolution\Benchmark"
set "location=%SYSTEMDRIVE%\Kon OS\Modules\Timer Resolution\Benchmark"
curl -s -L "%file%" -o "%location%\%name%"
...
:: Disables live tiles, smooth scrolling, folder templates, fast user switching, and ink related features. This makes Windows just feel overall faster, also flushes explorer.
...
cls && echo Optimizing power usage and latency via regedit...
:: Power settings
...
:: Configure ShutUp10
echo 🛈 Disabling telemetry, ads, and other sorts of crapware...
"%SYSTEMDRIVE%\Kon OS\Modules\OOShutUp10\OOSU10.exe" "%SYSTEMDRIVE%\Kon OS\Modules\OOShutUp10\config.cfg" /quiet
timeout /t 2 >nul
echo [92m✓ Done.
chcp 437 >nul
timeout /t 1 >nul
...
for %%M in (
    ...
    sc config %%M start= demand >nul 2>&1 && echo [97m[ [32mOK [97m] %%M changed successfully[39m || echo [97m[ [91mFAILED [97m] Failed to change %%M![39m
)
...
for %%Q in (
    ...
    sc config %%Q start= delayed-auto >nul 2>&1 && echo [97m[ [32mOK [97m] %%Q changed successfully[39m || echo [97m[ [91mFAILED [97m] Failed to change %%Q![39m
)
...
for %%A in (
    ...
    sc config %%A start= auto >nul 2>&1 && echo [97m[ [32mOK [97m] %%A changed successfully[39m || echo [97m[ [91mFAILED [97m] Failed to change %%A![39m
)
...
schtasks /DELETE /TN "Microsoft\Windows\WindowsUpdate\"
schtasks /DELETE /TN "Microsoft\Windows\InstallService\"
schtasks /DELETE /TN "Microsoft\Windows\UpdateOrchestrator"
...
rd /s /q %SYSTEMDRIVE%\Program Files (x86)\Internet Explorer
...
rd /s /q "%SYSTEMDRIVE%\Program Files (x86)\Microsoft\Edge" >nul 2>&1
rd /s /q "%SYSTEMDRIVE%\Program Files (x86)\Microsoft\EdgeCore" >nul 2>&1
rd /s /q "%SYSTEMDRIVE%\Program Files (x86)\Microsoft\EdgeUpdate" >nul 2>&1
rd /s /q "%SYSTEMDRIVE%\Program Files (x86)\Microsoft\EdgeWebView" >nul 2>&1
rd /s /q "%SYSTEMDRIVE%\Program Files (x86)\Microsoft\Temp" >nul 2>&1
:: Remove desktop shortcuts (user + public)
echo Deleting Edge shortcuts...
del "%SYSTEMDRIVE%\Users\Public\Desktop\Microsoft Edge.lnk" /f >nul 2>&1
...
netsh advfirewall firewall add rule name="Block_EdgeUpdate" dir=out action=block program="%SYSTEMDRIVE%\Program Files (x86)\Microsoft\EdgeUpdate\MicrosoftEdgeUpdate.exe" enable=yes >nul 2>&1
netsh advfirewall firewall add rule name="Block_EdgeUpdate_Inbound" dir=in action=block program="%SYSTEMDRIVE%\Program Files (x86)\Microsoft\EdgeUpdate\MicrosoftEdgeUpdate.exe" enable=yes >nul 2>&1
:: Blocks msedge.exe just in case all else fails
netsh advfirewall firewall add rule name="Block_msedge" dir=out action=block program="%SYSTEMDRIVE%\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" enable=yes >nul 2>&1
netsh advfirewall firewall add rule name="Block_msedge_Inbound" dir=in action=block program="%SYSTEMDRIVE%\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" enable=yes >nul 2>&1
echo Edge removed.
timeout /t 1 >nul
goto :WifiChoice
...
color c
set "smartscreen1=%SYSTEMDRIVE%\Windows\System32\smartscreen.exe"
set "smartscreen2=%SYSTEMDRIVE%\Windows\SystemApps\Microsoft.Windows.AppRep.ChxApp_cw5n1h2txyewy\CHXSmartScreen.exe"
...
set "location=%SYSTEMDRIVE%\Kon OS\temp"
mkdir "%location%" >nul 2>&1
curl -s -L "%file%" -o "%location%\%name%"
:: Download Open-Shell registry config (you already have this part)
:: Assume it's saved at %SYSTEMDRIVE%\Kon OS\Modules\OpenShell\KON.reg

:: Install Open-Shell
echo 🖥️ Installing Open-Shell...
start "" "%location%\%name%" /quiet ADDLOCAL=StartMenu
REM timeout /t 8 >nul (test to see if it applies with 0 delay)
chcp 65001 >nul
echo [92m✓ Done.
chcp 437 >nul
cls
echo ⬇️ Importing Open-Shell config...
...
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "TimerResolution" /t REG_SZ /d "%SYSTEMDRIVE%\Kon OS\Tools\Timer Resolution\SetTimerResolution.exe --resolution %resolution% --no-console" /f
chcp 65001 >nul
echo [92m✓ Done.
timeout /t 1 >nul 
goto :Wallpaper
...
mkdir "%SYSTEMDRIVE%\Kon OS\Modules\Themes\Wallpapers"
set "location=%SYSTEMDRIVE%\Kon OS\Modules\Themes\Wallpapers"
curl -s -L "%file%" -o "%location%\%name%"
:: Applying wallpaper
reg add "HKCU\Control Panel\Desktop" /v Wallpaper /t REG_SZ /d "%SYSTEMDRIVE%\Kon OS\Modules\Themes\Wallpapers\konosdark.jpg" /f
:: Lock Screen
set "file=https://i.imgur.com/hpcJ55a.jpeg"
set "name=konLS.jpg"
mkdir "%SYSTEMDRIVE%\Kon OS\Modules\Themes\Wallpapers\Lock Screen\"
set "location=%SYSTEMDRIVE%\Kon OS\Modules\Themes\Wallpapers\Lock Screen"
curl -s -L "%file%" -o "%location%\%name%"
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Personalization" /v LockScreenImage /t REG_SZ /d "%SYSTEMDRIVE%\Kon OS\Modules\Themes\Wallpapers\Lock Screen\konLS.jpg" /f
...
:: Extra Themes
cls
echo 🖥️ Installing extra themes...
...
set "location=%SYSTEMDRIVE%\Kon OS\Modules\Themes"
curl -s -L "%file%" -o "%location%\%name%"
...
set "name=OEM.bmp"
mkdir "%SYSTEMDRIVE%\Kon OS\Modules\Other"
set "location=%SYSTEMDRIVE%\Kon OS\Modules\Other"
curl -s -L "%file%" -o "%location%\%name%"
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\OEMInformation" /v Logo /t REG_SZ /d "%SYSTEMDRIVE%\Kon OS\Modules\Other\OEM.bmp" /f

cls && echo Adding shortcuts...
chcp 437 >nul
PowerShell -ExecutionPolicy Bypass -NoProfile -File "%~dp0Modules\Scripts\createShortcuts.ps1"
chcp 65001 >nul
echo Config shortcut added to desktop.
timeout /t 1 >nul 

:: Implementing Post Setup Configurator NEEGAH
set "file=https://raw.githubusercontent.com/ki8y/Tweaks/main/Modules/PostSetup.bat"
set "name=PostSetup.bat"
mkdir "%SYSTEMDRIVE%\Kon OS\Modules\PostSetup"
set "location=%SYSTEMDRIVE%\Kon OS\Modules\PostSetup"
curl -s -L "%file%" -o "%location%\%name%"
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "PostSetup" /t REG_SZ /d "%SYSTEMDRIVE%\Kon OS\Modules\PostSetup\PostSetup.bat" /f
...
rd /s /q "%SYSTEMDRIVE%\Kon OS\temp" >nul 2>&1
echo 🛈 Flushing DNS...
...
del "%SYSTEMDRIVE%\Windows\System32\SleepStudy" /s /f /q >nul 2>&1
